...
public class OrcTester
{
            ...
            throws Exception
    {
        ...
        for (CompressionKind compression : compressions) {
            boolean hiveSupported = (compression != LZ4) && (compression != ZSTD)
                    ...
                    && !isTime(writeType) && !isTime(readType);
    ...
    private static boolean isTime(Type type)
    {
        if (type.equals(TIME_MICROS)) {
            return true;
        }
        if (type instanceof ArrayType) {
            return isTime(((ArrayType) type).getElementType());
        }
        if (type instanceof MapType) {
            return isTime(((MapType) type).getKeyType()) || isTime(((MapType) type).getValueType());
        }
        if (type instanceof RowType) {
            return ((RowType) type).getFields().stream()
                .map(RowType.Field::getType)
                .anyMatch(OrcTester::isTime);
        }
        return false;
    }
